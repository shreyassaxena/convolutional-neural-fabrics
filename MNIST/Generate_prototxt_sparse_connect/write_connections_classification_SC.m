function [] = write_connections_classification_SC(fileID, flag_outgoing, flag_incoming, net_matrix, layer_index, depth_index, info_net)
...
% Here we are going to implement sparse connect
...
if(flag_incoming.forward )
    ...
    if(flag_skip)
        % We don't do the sparse connect for this case.
        string_forward = [string_forward,'\nconvolution_param {num_output: ',num2str(channels_at_depth),' pad: ',num2str(number_padding_MNIST),' kernel_size: 3 weight_filler {type: "xavier" std: 0.01 } bias_filler{type: "constant" value: 0}  engine:CAFFE group:1}} \n'];
    else
        string_forward = [string_forward,'\nconvolution_param {num_output: ',num2str(channels_at_depth),' pad: ',num2str(number_padding_MNIST),' kernel_size: 3 sparse_connect: true  weight_filler {type: "xavier" std: 0.01 } bias_filler{type: "constant" value: 0}  engine:CAFFE group:1}} \n'];
    end
...
if(flag_incoming.down)
    ...
    string_down = [string_down ,'\nconvolution_param {num_output: ',num2str(channels_at_depth),' pad: 1 kernel_size: 3 sparse_connect: true  weight_filler {type: "xavier" std: 0.01 } bias_filler{type: "constant" value: 0}  engine:CAFFE group:1}} \n'];
    fprintf(fileID,string_down);
end
...
if(flag_incoming.up)
    ...
    string_up = [string_up,'\nconvolution_param {num_output: ',num2str(channels_at_depth),' stride: 2 pad: 1 kernel_size: 3 sparse_connect: true  weight_filler {type: "xavier" std: 0.01 } bias_filler{type: "constant" value: 0}  engine:CAFFE group:1}} \n'];
    fprintf(fileID,string_up);
end
