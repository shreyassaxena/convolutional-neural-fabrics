function [ ] = generate_prototxt_full_stack_MNIST_image_list(number_layers, number_channels)
% This code was copied from VOC2007/ generate_prototxt_full_stack_v1_without_multiple_scale .
% That was the case where we do not have multiple scale inputs, and also we perform strided convolution
% for downsampling.

% ORIGINAL:
% The major change in this case is that for down-pooling we will use
% convolution with a stride-2, so here it is a convolution followed by
% selective sampling. Whereas in other cases we were doing convolution at
% node and then adding.
% In this case, the only input to the first nodes at each depth would be
% the zipper coming from above.
%
% CHANGES:
% 0: We have to convert the net for a classification task, so the zipper at
% the end has to come down instead of up.
% 1: The input will be 28x28 image, but we would make it 32x32 by zero padding it in the code.
%    Max depth: 6
% 2: The net would be doing classification. So we have to hook up a FC at the end of the lowest scale now
% followed by a SOFTMAX_LOSS!
% 3: Use the same accuracy measure used.


% The change in this case is that we will use the image list to feed the
% CNN. The image list already contains the distorted images which are
% saved as 32x32. So we will change the starting header and the pad size
% for the first input convolutional layer, we don't need any special
% padding.
...
% Open a file
fileID = fopen(['6x',num2str(number_layers),'_',num2str(number_channels),'channels.prototxt'],'w');
   ...
   % Step one by one on layer, except the last, since we have to zip it up
    ...
    for j = 1:info_net.depth
        ...
        if(net_matrix(j,i))
            ...
            write_connections_classification(fileID, flag_outgoing, flag_incoming, net_matrix, i, j, info_net);
        end
...
if(is_CPU)
    fileID = fopen(['CPU_solver_6x',num2str(number_layers),'_',num2str(number_channels),'channels_LR_0.01.prototxt'],'w');
else
    fileID = fopen(['GPU_solver_6x',num2str(number_layers),'_',num2str(number_channels),'channels_LR_0.01.prototxt'],'w');    
end

my_print(fileID,'# The train/test net protocol buffer definition') 
my_print(fileID,['net: "./','6x',num2str(number_layers),'_',num2str(number_channels),'channels.prototxt','"'])
my_print(fileID,'test_iter: 1000')
%my_print(fileID,'test_interval: 500')
my_print(fileID,'test_interval: 20000')
my_print(fileID,'')
...
my_print(fileID,'# Display every 100 iterations')
my_print(fileID,'display: 100')
my_print(fileID,'max_iter: 20000')
my_print(fileID,'snapshot: 2000')
my_print(fileID,['snapshot_prefix: "./6x',num2str(number_layers),'_',num2str(number_channels),'channels_LR_0.01_"'])
