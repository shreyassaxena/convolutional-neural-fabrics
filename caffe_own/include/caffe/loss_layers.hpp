...
// This layer is derived from SoftmaxWithLossLayer, and we have added the functionality of weighting 
// the loss and the gradient by the number of labels present in each instance.
template <typename Dtype>
class SoftmaxWithWeightedLossLayer : public LossLayer<Dtype> {
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_SOFTMAX_LOSS_WEIGHTED;
  }
  virtual inline int ExactNumBottomBlobs() const { return -1; }
  virtual inline int MinBottomBlobs() const { return 2; }
  virtual inline int MaxBottomBlobs() const { return 3; }
