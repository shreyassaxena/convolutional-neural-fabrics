...
/**
 * @brief Provides data to the Net from image files, but also outputs their x,y cropped parts
 *
 * TODO(dox): thorough documentation for Forward and proto params.
 */
template <typename Dtype>
class ImageDataCropOwnLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit ImageDataCropOwnLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~ImageDataCropOwnLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
