...
template<typename Dtype>
void DataTransformer<Dtype>::Transform_coord(const cv::Mat& cv_img,
                                       Blob<Dtype>* transformed_blob, int& coord_x, int& coord_y) {
  const int img_channels = cv_img.channels();
  const int img_height = cv_img.rows;
  const int img_width = cv_img.cols;
 ...
 // We have to pass back this value of h_off and w_off
 //coords_.assign(2, 0); 
 coord_x = w_off;
 coord_y = h_off;
