...
void SoftmaxLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  scale_.Reshape(bottom[0]->num(), 1, bottom[0]->height(), bottom[0]->width());          // Where is this scale_ coming from?
}
...
void SoftmaxLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num; ++i) {
    ...
    caffe_copy(spatial_dim, bottom_data + i * dim, scale_data); // Here scale data is basically reading the first channel of the nth image?
    for (int j = 0; j < channels; j++) {
      for (int k = 0; k < spatial_dim; k++) {
        scale_data[k] = std::max(scale_data[k],                     // scale_data in the end will contain the maximum value of a pixel across the channels and number of data points
