...
void ConvolutionLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* weight_mutable = this->blobs_[0]->mutable_gpu_data();         // These are the weights which should be modified

    if(this->is_sparse_connect_ || this->is_sparse_connect_higher_to_lower || this->is_sparse_connect_lower_to_higher){
      caffe_gpu_mul(this->blanking_blob_.count(), weight, this->blanking_blob_.gpu_data(), weight_mutable);
    }
...
if(this->is_sparse_connect_ || this->is_sparse_connect_higher_to_lower || this->is_sparse_connect_lower_to_higher){
      caffe_gpu_mul(this->blanking_blob_.count(), weight_diff, this->blanking_blob_.gpu_data(), weight_diff);
}
