...
template <typename Dtype>
__global__ void TopLeftForward(const int nthreads, const Dtype* bottom_data,
    const int num, const int channels, const int height,
    const int width, const int pooled_height, const int pooled_width,
    const int kernel_h, const int kernel_w, const int stride_h,
    const int stride_w, const int pad_h, const int pad_w, Dtype* top_data,
...
void PoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  const bool use_top_mask = top.size() > 1;                            // This is the reason why top_mask was outputted in AVE pooling!
  int* mask = NULL;
  Dtype* top_mask = NULL;
  ...
  case PoolingParameter_PoolMethod_STOCHASTIC:
    ...
    break;
  case PoolingParameter_PoolMethod_TOPLEFT:
    if (use_top_mask) {
      top_mask = top[1]->mutable_gpu_data();
    } else {
      mask = max_idx_.mutable_gpu_data();
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    TopLeftForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, bottom_data, bottom[0]->num(), channels_,
        height_, width_, pooled_height_, pooled_width_, kernel_h_,
        kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_, top_data,
        mask, top_mask);
...
template <typename Dtype>
__global__ void TopLeftBackward(const int nthreads, const Dtype* top_diff,                    // Remains the same as the normal  MAXPOOL case, since the only thing different is the switches
    const int* mask, const Dtype* top_mask, const int num, const int channels,
    const int height, const int width, const int pooled_height,
    const int pooled_width, const int kernel_h, const int kernel_w,
    const int stride_h, const int stride_w, const int pad_h, const int pad_w,
...
void PoolingLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  case PoolingParameter_PoolMethod_TOPLEFT:
    ...
    // NOLINT_NEXT_LINE(whitespace/operators)
    TopLeftBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top_diff, mask, top_mask, top[0]->num(), channels_,
        height_, width_, pooled_height_, pooled_width_,
        kernel_h_, kernel_w_, stride_h_, stride_w_, pad_h_, pad_w_,
        bottom_diff);
