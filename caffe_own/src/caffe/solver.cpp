...
void Solver<Dtype>::Step(int iters) {
  ...
  for (; iter_ < stop_iter; ++iter_) {
    // zero-init the params ( We do this for the gradients )
    ...
    for (int i = 0; i < param_.iter_size(); ++i) {
      loss += net_->ForwardBackward(bottom_vec);             //Shreyas: We do not touch the parameters here, just compute the loss and update the value in the diff!
    }
    ...
    // This adds to the gradient the update value depending on the regularization and the Learning rate
    ComputeUpdateValue();
